...
| E-02: Executable draft contracts | CI-02 | Done as draft | TypeScript contracts, draft JSON Schema, CLI/API runtime validation including unknown-limit rejection, and error fixtures |
...
| E-09: Package and release gates | CI-09 | In progress | Local pack dry-run includes the unreleased changelog, `npm run release:check` validates package metadata and the tarball file set, dependency audit and installed API/CLI smoke are configured, and CI uses full-history checkout with `contents: read`; schema freeze, cross-platform artifact checks, and registry evidence remain |
