...
the full 24-case Node 22/24 Linux container runs, plus type checks and package
checks, pass with locked host dependencies mounted read-only. Earlier clean
Linux runs also pass the 23-case `npm ci` baseline and tarball install/API
smoke. The latest full run includes the external-helper fixture.
