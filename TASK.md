...
on Node.js `23.10.0` / macOS `Darwin 25.6.0 arm64` passes 30 smoke/integration
...
| CI-08 | Enforce budgets, deterministic results, isolation, and measured limits | In progress | CI-05, CI-06, CI-07 | Local limits/determinism/read-only checks, bounded fan-out API/CLI measurements across macOS and Node 22/24 Linux, and a high-fan-out unresolved-observation cap regression pass; Windows/hosted measurements, cancellation, and memory isolation remain; V-09, V-15 through V-20 |
...
revision `32fd01a` (documentation reconciliation before this update `1a57175`)
using Node.js
...
| Command | Result |
| --- | --- |
| `npm test` | Pass: 30 tests; build plus Node test runner |
