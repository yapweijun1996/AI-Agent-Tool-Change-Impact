...
Final local gate run on 2026-09-07 at code revision `149e0fa` used Node.js
...
the macOS runtime. The full package gate above remains the recorded gate at
`149e0fa`; no hosted or registry evidence is inferred from this targeted run.
...
| V-15 | Large files/projects, fan-out/deep graph, cancellation, repeated sessions | Partial | Four fan-out/depth sizes (21–501 files) on macOS plus current 31-case Node 22/24 Linux checks confirm default versus hard-cap behavior; bounded source/Git/external reads stop before decoding beyond the per-file budget; 12,000-missing-import and 20,000-oversized-file reproductions stay within the 300-observation and 1,000-diagnostic default budgets and emit explicit truncation markers; three repeated macOS cold starts preserve counts/stop reasons; cancellation and memory-isolation evidence remain |
