...
`git diff --check` all passed. The test count is now 30/30 after the provider
observation-bound regression coverage. This is local evidence; it does not substitute
...
| Bounded resource observation | `node spike/performance-benchmark.cjs` on temporary 21-, 121-, 241-, and 501-file fan-out/depth repositories on macOS, plus the 241-file fixture on Node 22/24 Linux containers; three repeated 241-file cold starts on macOS; a 12,000-missing-import provider reproduction under default limits | Pass locally: default node cap stops at 100 nodes for larger fixtures; hard caps complete; semantic counts and stop reasons remain stable across three repeated runs; the high-fan-out reproduction returns 299 unresolved observations with `PROVIDER_OBSERVATION_LIMIT` in a 163,378-byte envelope; API/CLI child-process timings and RSS across macOS/Linux are recorded in [`spike/PERFORMANCE_FINDINGS.md`](spike/PERFORMANCE_FINDINGS.md) |
...
| V-15 | Large files/projects, fan-out/deep graph, cancellation, repeated sessions | Partial | Four fan-out/depth sizes (21–501 files) on macOS plus current 30-case Node 22/24 Linux checks confirm default versus hard-cap behavior; a 12,000-missing-import reproduction stays within the 300-observation default budget and emits `PROVIDER_OBSERVATION_LIMIT`; three repeated macOS cold starts preserve counts/stop reasons; cancellation and memory-isolation evidence remain |
